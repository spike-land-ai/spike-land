...
export function createSpacetimeDb(_conn: unknown): ImageStudioDeps["db"] {
  return {
    ...
    async imageCount(userId) {
      return Array.from(tables.image.iter()).filter((img) => img.userIdentity === userId).length;
    },
    ...
    async jobCreate(data) {
      ...
      while (Date.now() - start < 5000) {
        const found = Array.from(tables.enhancement_job.iter()).find(
          (j) =>
            j.imageId === data.imageId &&
            j.userIdentity === data.userId &&
    ...
    async albumFindMany(opts) {
      const results = Array.from(tables.album.iter()).filter((a) => a.userIdentity === opts.userId);
    ...
    async albumMaxSortOrder(userId) {
      const results = Array.from(tables.album.iter()).filter((a) => a.userIdentity === userId);
    ...
    async pipelineCreate(data) {
      ...
      while (Date.now() - start < 5000) {
        const found = Array.from(tables.pipeline.iter()).find(
          (p) =>
            p.name === data.name &&
            (data.userId ? p.userIdentity && p.userIdentity === data.userId : !p.userIdentity),
        );
    ...
    async generationJobCreate(data) {
      ...
      while (Date.now() - start < 5000) {
        const found = Array.from(tables.generation_job.iter()).find(
          (j) =>
            j.prompt === data.prompt && j.userIdentity === data.userId && j.status === data.status,
        );
    ...
    async subjectCreate(data) {
      ...
      while (Date.now() - start < 5000) {
        const found = Array.from(tables.subject.iter()).find(
          (s) =>
            s.imageId === data.imageId && s.label === data.label && s.userIdentity === data.userId,
        );
...
export function createSpacetimeCredits(_conn: unknown): ImageStudioDeps["credits"] {
  return {
    async hasEnough(userId, amount) {
      const creds = Array.from(tables.credits.iter()).find((c) => c.userIdentity === userId);
      return (creds?.balance ?? 0n) >= BigInt(amount);
    },

    async consume(opts) {
      const creds = Array.from(tables.credits.iter()).find((c) => c.userIdentity === opts.userId);
      if (!creds || creds.balance < BigInt(opts.amount)) {
        return {
          success: false,
          remaining: Number(creds?.balance ?? 0n),
