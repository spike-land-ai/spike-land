...
// --- TypeScript compiler API type aliases ---
// These use `import type` which is erased at compile time and adds nothing to the bundle.
type TSCompletionInfo = ts.CompletionInfo;
type TSCompletionEntryDetails = ts.CompletionEntryDetails;
type TSSignatureHelpItemsOptions = ts.SignatureHelpItemsOptions;
type TSSignatureHelpItems = ts.SignatureHelpItems;
type TSQuickInfo = ts.QuickInfo;
type TSDocumentHighlights = ts.DocumentHighlights;
type TSDefinitionInfo = ts.DefinitionInfo;
type TSReferenceEntry = ts.ReferenceEntry;
type TSNavigationTree = ts.NavigationTree;
type TSFormatCodeOptions = ts.FormatCodeOptions;
type TSTextChange = ts.TextChange;
type TSRenameLocation = ts.RenameLocation;
type TSRenameInfoOptions = ts.RenameInfoOptions;
type TSRenameInfo = ts.RenameInfo;
type TSCodeFixAction = ts.CodeFixAction;
type TSInlayHint = ts.InlayHint;

export interface TypeScriptWorker {
  // The following type aliases map to TypeScript compiler API types via `import type`.
  ...
  getCompletionsAtPosition(
    fileName: string,
    position: number,
  ): Promise<TSCompletionInfo | undefined>;
  ...
  getCompletionEntryDetails(
    ...
    position: number,
    entry: string,
  ): Promise<TSCompletionEntryDetails | undefined>;
  ...
  getSignatureHelpItems(
    fileName: string,
    position: number,
    options: TSSignatureHelpItemsOptions,
  ): Promise<TSSignatureHelpItems | undefined>;

  /**
   * Get quick info for the item at the given position in the file.
   * @returns `Promise<typescript.QuickInfo | undefined>`
   */
  getQuickInfoAtPosition(fileName: string, position: number): Promise<TSQuickInfo | undefined>;

  getDocumentHighlights(
    fileName: string,
    position: number,
    filesToSearch: string[],
  ): Promise<ReadonlyArray<TSDocumentHighlights> | undefined>;
  ...
  getDefinitionAtPosition(
    fileName: string,
    position: number,
  ): Promise<ReadonlyArray<TSDefinitionInfo> | undefined>;
   ...
   */
  getNavigationTree(fileName: string): Promise<TSNavigationTree | undefined>;
  ...
  getFormattingEditsForDocument(
    fileName: string,
    options: TSFormatCodeOptions,
  ): Promise<TSTextChange[]>;
  ...
  ): Promise<readonly TSRenameLocation[] | undefined>;
  ...
  getRenameInfo(
    fileName: string,
    positon: number,
    options: TSRenameInfoOptions,
  ): Promise<TSRenameInfo>;
  ...
  getCodeFixesAtPosition(
    ...
    errorCodes: number[],
    formatOptions: TSFormatCodeOptions,
  ): Promise<ReadonlyArray<TSCodeFixAction>>;
  ...
  ): Promise<ReadonlyArray<TSInlayHint>>;
}
