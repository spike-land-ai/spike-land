...
const StreamExists = ReadableStreamExists && WritableStreamExists && TransformStreamExists;
...
export async function isSupported(): Promise<{
  ...
  const getChannels = async () => {
    ...
    await new Promise<void>((resolve) => {
      messageChannel.port1.postMessage(obj1, [obj1.port1]);
      messageChannel.port1.onmessage = () => {
        resolve();
      };
      messageChannel.port2.onmessage = ({ data }) => {
        messageChannel.port2.postMessage(data, [data.port1]);
      };
    });
