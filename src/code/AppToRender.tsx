...
const ActionButtons: FC<ActionButtonsProps> = memo(({ isOpen, onToggleChat, onToggleHistory }) => {
  return (
    <div className="fixed bottom-4 right-4 flex flex-col gap-2 z-[1001]">
      <Button
        onClick={onToggleHistory}
        className={cn("rounded-full w-12 h-12 p-0", "hover:bg-primary transition-colors")}
...
export const AppToRender: FC<AppComponentProps> = memo(
  ({
    ...
    // Chat toggle state
    const [isOpen, setIsOpen] = useState(Boolean(maybeKey && sessionStorage.getItem(maybeKey)));
