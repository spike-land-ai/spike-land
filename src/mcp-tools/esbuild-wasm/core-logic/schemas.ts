...
const SourcemapEnum = z.union([z.boolean(), z.enum(["linked", "inline", "external", "both"])]);
...
export const BuildOnlySchema = {
  ...
  entryNames: z
    .string()
    .optional()
    .describe('Template for entry point output names (e.g. "[dir]/[name]-[hash]")'),
  ...
  external: z
    .array(z.string())
    .optional()
    .describe("Package names or globs to exclude from bundle"),
