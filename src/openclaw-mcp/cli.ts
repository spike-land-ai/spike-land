...
process.on('unhandledRejection', (err: unknown) => shipper.shipError({ service_name: "openclaw-mcp", message: (err as Error)?.message || String(err), stack_trace: (err as Error)?.stack, severity: "high" }));
