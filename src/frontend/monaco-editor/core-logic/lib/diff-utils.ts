...
export function extractDiffContent(content: string): { original: string; modified: string } {
  ...
  if (searchIndex !== -1) {
    ...
    if (separatorIndex !== -1) {
      original = content
        .slice(searchIndex + searchMarker.length, separatorIndex)
        .split(separatorMarker)
        .join("")
        .trim();
      if (replaceIndex !== -1) {
        modified = content
          .slice(separatorIndex + separatorMarker.length, replaceIndex)
          .split(separatorMarker)
          .join("")
          .trim();
      } else {
        modified = content
          .slice(separatorIndex + separatorMarker.length)
          .split(separatorMarker)
          .join("")
          .trim();
      }
      ...
      original = content
        .slice(searchIndex + searchMarker.length)
        .split(separatorMarker)
        .join("")
        .trim();
    }
...
function findBestLineByLineMatch(
  ...
  if (!bestAnchorLine) {
    // Handle case where significantSearchLines is empty
    return null;
  }
...
export function replacePreservingWhitespace(text: string, search: string, replace: string): string {
  ...
  if (search.includes("\n")) {
    ...
    if (lineByLineMatch) {
      return (
        text.substring(0, lineByLineMatch.startIndex) +
        replace +
