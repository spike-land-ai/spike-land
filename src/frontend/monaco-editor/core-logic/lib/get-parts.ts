import type { LanguageMap, ParsingState } from "../../ui/@/lib/interfaces";
  ...
  if (isUser) {
    ...
    return (
      (text || "")
        ...
        .pop()
        ?.trim()
        .split("Reminder from the system:")[0] || "".trim()
    );
  }
  return text;
};
...
const extendTextWithDiffMarkers = (text: string, state: ParsingState): string => {
  let result = "";
  let index = 0;
