import type { RenderedApp } from "../ui/@/lib/interfaces";
...
export class RenderService {
   ...
   */
  public async handleRender(renderedNew: RenderedApp | null): Promise<RenderResult | false> {
    if (renderedNew === null) {
      return {
        css: "",
        html: "",
    ...
    const emotionGlobalStyles = Array.from(
      ...
      (x.sheet ? Array.from(x.sheet.cssRules).map((rule: CSSRule) => rule.cssText) : []).join("\n"),
