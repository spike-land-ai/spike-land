...
import AlwaysSupportedSharedWorker from "../core-logic/lib/shared-w-polyfill";
import { getTransferables, hasTransferables } from "../core-logic/lib/transferables";
import type { MyBuildOptions } from "./transpile";
...
class WorkerPool {
  ...
  getWorker(tag = "default") {
    ...
    const availableWorker =
      this.workers.find((worker) => !worker.busy && worker.tag === tag) || this.addWorker(tag);
...
async function init() {
  workerPool =
    (globalThis as unknown as { workerPool: WorkerPool }).workerPool || new WorkerPool(0);
...
export const tsx = async (code: string): Promise<Array<{ content: string; filePath: string }>> => {
