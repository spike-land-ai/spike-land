...
const initializeModule = async (wasmModule?: WebAssembly.Module, origin?: string) => {
  ...
  if (wasmModule) {
    ...
    // Try IDB-cached WASM first
    const cachedWasm = await getWasmModule("esbuild-wasm");
    if (cachedWasm) {
      const compiled = await WebAssembly.compile(cachedWasm);
      await mod.initialize(compiled);
    } else {
      const wasmURL = new URL(`${origin}/${wasmFile}`).toString();
...
const getDefaultBuildOptions = ({
  ...
  entryPoints: entryPoints ? [entryPoints] : [`${origin}/live/${codeSpace}/wrapper.js`],
...
export const build = async ({
  ...
  return mutex.runExclusive(async () => {
    const buildPromise = async () => {
      ...
      const currentBuildOptions = {
        // Renamed to avoid conflict
        ...defaultOpts,
        external: [...Object.keys(importMap.imports)],
        entryPoints: entryPoints ? entryPoints : [`${origin}/live/${codeSpace}/index.tsx`],
      } as BuildOptions;
