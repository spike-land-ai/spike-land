...
const toHtmlAndCss = async (
...
): Promise<{ css: string; html: string }> => {
  {
    ...
    const html = rootElement ? htmlDecode(rootElement.innerHTML).split(cssCache.key).join("x") : "";
      ...
      Promise.resolve(
        Array.from(
          document.querySelectorAll<HTMLStyleElement>(
            // Changed to Array.from and removed .values()
    ...
    const tailWindClasses = tailwindStyles.map((z) => z.innerHTML).join("\n");

    const tailWindClassesXWithoutComments = tailWindClasses.replace(
      /\/\*[\s\S]*?\*\/|([^\\:]|^)\/\/.*$/gm,
      "",
    );
    const tailWindClassesX = tailWindClassesXWithoutComments
      .split(`\\\\[`)
      .join(`\\[`)
      .split(`\\\\]`)
      .join(`\\]`);
...
const renderedApps = ((globalThis as GlobalWithRenderedApps).renderedApps =
  (globalThis as GlobalWithRenderedApps).renderedApps || new WeakMap<HTMLElement, RenderedApp>());
...
export const importFromString = async (code: string) => {
  ...
  // Import the file
  const { data: module, error: importError } = await tryCatch(import(/* @vite-ignore */ filePath));
...
async function _handleTestEnvironment({
  ...
  transpiled,
  code,
}: Pick<IRenderApp, "App" | "transpiled" | "code">): Promise<RenderedApp> {
...
async function _loadAppComponent(
  ...
  if (App) {
    ...
    const { data: resp } = await tryCatch(fetch(`${currentOrigin}/live/${codeSpace}/session.json`));
    ...
    if (sessionTranspiled) {
      ...
      AppToRender =
        importErr || !mod ? FallbackErrorComponent : (mod.default as FlexibleComponentType);
    }
...
async function renderApp(props: IRenderApp): Promise<RenderedApp> {
