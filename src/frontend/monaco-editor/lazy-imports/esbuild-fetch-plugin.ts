    ...
    setup(build) {
      ...
      build.onLoad({ filter: /.*/, namespace: "http-url" }, async (args) => {
        ...
        if (
          ...
          !(
            argsPath.endsWith(".mjs") ||
            argsPath.endsWith(".js") ||
            argsPath.endsWith(".json") ||
            argsPath.endsWith(".css") ||
            argsPath.endsWith(".tsx") ||
        ...
        if (argsPath === args.path && !args.path.includes(origin)) {
          argsPath = `/*${args.path}?bundle=true&external=${EXTERNAL_DEPENDENCIES.join(
            ",",
          )}&${DEPS_PARAM}`;
        }
 ...
 */
async function processCSS(css: string, baseURL: string, depth = 0): Promise<string> {
