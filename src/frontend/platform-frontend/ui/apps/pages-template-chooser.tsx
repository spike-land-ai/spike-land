...
import {
  PAGES_TEMPLATE_SEEDS,
  type PagesTemplateSeed,
  type TemplateCategoryId,
} from "./pages-template-chooser-data";

type SidebarCategoryId = "all" | "premium" | "recent" | "favorites" | TemplateCategoryId;
...
const STORAGE_KEY_RECENTS = "pages-template-chooser-recents";
const STORAGE_KEY_FAVORITES = "pages-template-chooser-favorites";
const MAX_RECENTS = 8;
...
function inferTone(template: PagesTemplateSeed): {
  ...
  if (
    ...
    haystack.includes("gradient") ||
    haystack.includes("recreation")
  ) {
    return {
  ...
  if (
    haystack.includes("educator") ||
    haystack.includes("school") ||
    haystack.includes("note taking") ||
...
function TemplatePreview({
  ...
  return (
    <div
      className={cn(
        "group/preview relative overflow-hidden rounded-[22px] border p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.45)]",
    ...
    >
      <button
        type="button"
        onClick={onTogglePin}
        aria-label={pinned ? "Unpin favorite" : "Pin as favorite"}
        className="absolute left-3 top-3 z-10 flex h-7 w-7 items-center justify-center rounded-[10px] border border-white/25 bg-black/70 transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e47d2f] focus-visible:ring-offset-2 group-hover/preview:opacity-100"
        style={{ opacity: pinned ? 1 : undefined }}
      >
        <Heart
          className={cn("h-3.5 w-3.5", pinned ? "fill-red-500 text-red-500" : "text-white/70")}
        />
      </button>
      ...
      <div
        className="mx-auto min-h-[176px] rounded-[18px] border bg-white/78 p-3 transition-transform duration-200 group-hover:scale-[1.03]"
...
export function PagesTemplateChooserApp() {
  ...
  const [recentIds, setRecentIds] = useState<string[]>(() => readJsonArray(STORAGE_KEY_RECENTS));
  const [pinnedIds, setPinnedIds] = useState<Set<string>>(
    () => new Set(readJsonArray(STORAGE_KEY_FAVORITES)),
  );
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY_RECENTS, JSON.stringify(recentIds));
    } catch {
      /* quota */
    }
  }, [recentIds]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY_FAVORITES, JSON.stringify([...pinnedIds]));
    } catch {
      /* quota */
    }
  }, [pinnedIds]);
  ...
  const handleCreate = useCallback(
    (templateOverride?: PagesTemplateSeed) => {
      const t =
        templateOverride ??
        PAGES_TEMPLATE_SEEDS.find((tpl) => tpl.id === selectedTemplateId) ??
        null;
      if (!t) return;
      setSelectedTemplateId(t.id);
      setAnnouncement(`Creating document from ${t.name}`);
    },
    [selectedTemplateId],
  );
  ...
  const selectTemplate = useCallback((id: string) => {
    ...
    const t = PAGES_TEMPLATE_SEEDS.find((tpl) => tpl.id === id);
    if (t) {
      setAnnouncement(`Selected: ${t.name}${t.premium ? " (Premium)" : ""}`);
      setRecentIds((prev) => {
        const deduped = prev.filter((rid) => rid !== id);
        return [id, ...deduped].slice(0, MAX_RECENTS);
      });
    }
  }, []);
  ...
  const visibleTemplates = useMemo(() => {
    if (activeCategory === "recent") {
      const templates = recentIds
        .map((id) => PAGES_TEMPLATE_SEEDS.find((t) => t.id === id))
        .filter((t): t is PagesTemplateSeed => t != null);
      return deferredSearch ? templates.filter((t) => matchesSearch(t, deferredSearch)) : templates;
    }
    if (activeCategory === "favorites") {
      const templates = PAGES_TEMPLATE_SEEDS.filter((t) => pinnedIds.has(t.id));
      return deferredSearch ? templates.filter((t) => matchesSearch(t, deferredSearch)) : templates;
    }
    return PAGES_TEMPLATE_SEEDS.filter((template) => {
      ...
      if (
        ...
        !template.categories.includes(activeCategory as TemplateCategoryId)
      ) {
        return false;
      }
      return matchesSearch(template, deferredSearch);
    });
  }, [activeCategory, deferredSearch, recentIds, pinnedIds]);
  ...
  const currentLabel =
    SIDEBAR_CATEGORIES.find((category) => category.id === activeCategory)?.label ??
    "Choose a Template";
  ...
  const handleSidebarKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === "ArrowDown") {
      nextIndex = currentIndex < SIDEBAR_CATEGORIES.length - 1 ? currentIndex + 1 : 0;
    } else {
      nextIndex = currentIndex > 0 ? currentIndex - 1 : SIDEBAR_CATEGORIES.length - 1;
    }
    const nextId = SIDEBAR_CATEGORIES[nextIndex].id;
    startTransition(() => {
  ...
  const handleGridKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === "Enter" && focusedIndex >= 0) {
      event.preventDefault();
      const cardId = cards[focusedIndex].dataset.templateId;
      if (cardId) selectTemplate(cardId);
      return;
    }
    ...
    if (event.key === "ArrowRight") nextIndex = Math.min(focusedIndex + 1, cards.length - 1);
    else if (event.key === "ArrowLeft") nextIndex = Math.max(focusedIndex - 1, 0);
    else if (event.key === "ArrowDown") nextIndex = Math.min(focusedIndex + cols, cards.length - 1);
    else if (event.key === "ArrowUp") nextIndex = Math.max(focusedIndex - cols, 0);
    else return;
    ...
    if (activeCategory === "recent") {
      return (
        <div className="rounded-[28px] border border-dashed border-black/10 bg-white/70 p-10 text-center text-[#6b7280]">
          You haven&apos;t selected any templates yet.
        </div>
      );
    }
    if (activeCategory === "favorites") {
      return (
        <div className="rounded-[28px] border border-dashed border-black/10 bg-white/70 p-10 text-center text-[#6b7280]">
          Pin your favorite templates to see them here.
        </div>
      );
    }
    if (deferredSearch) {
      return (
        <div className="rounded-[28px] border border-dashed border-black/10 bg-white/70 p-10 text-center text-[#6b7280]">
          No templates match &ldquo;{search.trim()}&rdquo; in this category.
        </div>
      );
    }
    return (
  ...
  const renderTemplateCard = (template: PagesTemplateSeed) => {
    const isSelected = selectedTemplateId === template.id;
    const isPinned = pinnedIds.has(template.id);
    return (
      <div
        key={template.id}
        role="option"
        aria-selected={isSelected}
        aria-label={`${template.name}${template.premium ? " (Premium)" : ""}`}
        ...
        className={cn(
          "group cursor-pointer overflow-hidden rounded-[28px] border bg-white p-3 text-left shadow-[0_16px_48px_rgba(15,23,42,0.08)] transition-all hover:-translate-y-0.5 hover:border-[#e47d2f]/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e47d2f] focus-visible:ring-offset-2",
          isSelected
            ? "border-[#e47d2f] shadow-[0_20px_56px_rgba(228,125,47,0.24)]"
            : "border-black/6",
        )}
      >
        <TemplatePreview
            ...
            {template.premium && (
              <span
                className={cn(
                  ...
                  isSelected ? "bg-white/20 text-white" : "bg-[#1f2937] text-[#caa8ff]",
                )}
              >
                premium
              </span>
        ...
        <div className="flex items-center justify-between border-b border-black/5 px-6 py-4">
          ...
          <div className="hidden rounded-full bg-white/75 px-4 py-2 text-sm font-medium text-[#4b5563] shadow-sm md:block">
            {selectedTemplate
              ...
              : "Select a template to create a document"}
          </div>
        </div>
              ...
              {SIDEBAR_CATEGORIES.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e47d2f] focus-visible:ring-offset-2",
                      isActive
          ...
          <section className="flex min-h-0 flex-col bg-[#faf9fb]">
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-3xl text-base leading-7 text-[#5d6473]">
                    Browse a native-feeling Pages gallery with dense categories, premium badges, and
                    a clear creation path.
                  </p>
                </div>

                <label className="flex w-full items-center gap-3 rounded-2xl border border-black/6 bg-white px-4 py-3 shadow-sm focus-within:ring-2 focus-within:ring-[#e47d2f] lg:max-w-sm">
                  ...
                  <div className="grid gap-6 px-6 py-6 md:grid-cols-[1.2fr_0.8fr] md:items-center">
                    ...
                    <div className="rounded-[24px] border border-white/20 bg-white/12 p-5 backdrop-blur-md">
                      <div className="grid grid-cols-3 gap-3">
                        {PAGES_TEMPLATE_SEEDS.filter((template) => template.premium)
                          .slice(0, 3)
                          .map((template) => (
                            <div
                              key={template.id}
                  ...
                  <div className="space-y-10">
                    {groupedSections.map((section) => (
                      <section key={section.id} className="space-y-4">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-[#6b7280]">
                              {section.templates.length} template
                              {section.templates.length === 1 ? "" : "s"}
                            </p>
                          </div>
                        </div>
                        ...
                        <div
                          role="listbox"
                          aria-label={`${section.label} templates`}
            ...
            <div className="flex items-center justify-between border-t border-black/5 px-6 py-5">
              <button
                ...
                className="rounded-2xl border border-black/10 bg-white px-6 py-3 text-sm font-semibold text-[#253041] shadow-sm transition hover:bg-[#f5f5f8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e47d2f] focus-visible:ring-offset-2"
              >
                Cancel
              </button>
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className={cn(
                    "rounded-2xl px-8 py-3 text-sm font-bold text-white shadow-[0_18px_42px_rgba(62,104,233,0.25)] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e47d2f] focus-visible:ring-offset-2",
                    selectedTemplate
