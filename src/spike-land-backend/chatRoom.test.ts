  ...
  describe("updateAndBroadcastSession", () => {
    ...
    beforeEach(async () => {
      ...
      (mockState.storage.get as ReturnType<typeof vi.fn>).mockImplementation(
        async (key: string) => {
          if (key === "session_core") {
            return { codeSpace: roomName, messages: [] };
          }
    ...
    it("should overflow large code to R2 via LargeValueStorage", async () => {
      ...
      expect(mockEnv.R2.put).toHaveBeenCalledWith("do_test-id/session_code", largeCode);
    ...
    it("should overflow large transpiled to R2 via LargeValueStorage", async () => {
      ...
      expect(mockEnv.R2.put).toHaveBeenCalledWith("do_test-id/session_transpiled", largeTranspiled);
      expect(mockState.storage.put).toHaveBeenCalledWith(
        "session_transpiled",
        expect.objectContaining({
          __r2_ref: true,
