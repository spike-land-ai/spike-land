...
describe("PostHandler", () => {
  ...
  describe("handle", () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(streamText).mockResolvedValue(mockStreamResponse as StreamResult);

      // Mock createAnthropic to return a proper AnthropicProvider
      const mockTools = createMockTools();

      const anthropicProvider = Object.assign(vi.fn().mockReturnValue("claude-4-sonnet-20250514"), {
    ...
    it("should handle stream errors", async () => {
      ...
      const anthropicProvider = vi
        .fn()
        .mockReturnValue("claude-4-sonnet-20250514") as unknown as AnthropicProvider;
    ...
    it("should handle tool execution in onStepFinish", async () => {
      ...
      await onStepFinishCallback!({
        toolResults: [{ toolCallId: "1", result: { output: "test" } }],
        content: [],
        text: "",
        reasoning: [],
        reasoningText: "",
