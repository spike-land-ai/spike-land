...
export function Dashboard() {
  ...
  if (loading) {
    return (
      <div className="space-y-6 animate-pulse animate-delayed-show">
        <div className="h-8 w-48 bg-obsidian-900 border border-white/5 rounded-lg" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-32 bg-obsidian-900 border border-white/5 rounded-xl" />
          ))}
        </div>
      </div>
    );
