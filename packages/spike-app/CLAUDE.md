# CLAUDE.md

## Overview

Vite + React 19 + TanStack Router SPA replacing the Next.js UI (`spike.land`). Talks to `spike-edge` (via `/api` proxy) and SpacetimeDB directly over WebSocket. Published as `@spike-land-ai/spike-app`.

## Commands

```bash
npm run dev           # Vite dev server (proxies /api → localhost:8787)
npm run build         # tsc --noEmit && vite build
npm run preview       # Serve production build locally
npm run typecheck     # tsc --noEmit
npm run lint          # ESLint src/
npm test              # Vitest run
npm run test:coverage # Vitest with v8 coverage
```

## Architecture

```
src/
├── main.tsx              # App entry — AuthProvider, RouterProvider
├── routes/
│   ├── __root.tsx        # Root layout (sidebar nav, SpacetimeDB status indicator)
│   ├── index.tsx         # Dashboard
│   ├── analytics.tsx     # Analytics
│   ├── callback.tsx      # OIDC callback handler
│   ├── login.tsx         # Login page
│   ├── settings.tsx      # Settings
│   ├── store.tsx         # App store
│   ├── apps/             # Apps routes ($appId.tsx, index.tsx, new.tsx)
│   ├── messages/         # Messages routes ($userId.tsx, index.tsx)
│   └── tools/            # Tools routes ($category.tsx, index.tsx)
├── components/
│   ├── AppCard.tsx
│   ├── AuthGuard.tsx
│   ├── ChatThread.tsx
│   ├── LivePreview.tsx
│   ├── LoginButton.tsx
│   ├── StatusBadge.tsx
│   └── VersionHistory.tsx
├── hooks/
│   ├── useAnalytics.ts   # Page-view tracking via stdbClient.recordEvent
│   ├── useAuth.ts        # Wraps react-oidc-context
│   ├── useStdb.ts        # Subscribes to stdbClient connection state
│   └── useTable.ts       # SpacetimeDB table subscription hook
├── lib/
│   ├── stdb.ts           # Singleton WebSocket client (connect/disconnect/retry)
│   └── auth.ts           # OIDC config (react-oidc-context + oidc-client-ts)
├── routeTree.gen.ts      # Auto-generated by TanStack Router — do not edit
├── test-setup.ts         # Vitest + @testing-library/jest-dom setup
└── app.css               # Tailwind v4 base styles
```

### Routing

TanStack Router with file-based route tree. `routeTree.gen.ts` is auto-generated — run `npm run dev` to regenerate after adding routes. Route files live under `src/routes/`; nested directories create nested routes.

### State Management

No global state library. State lives in:
- **SpacetimeDB** — authoritative server state via `lib/stdb.ts` singleton (WebSocket, exponential-backoff reconnect)
- **React hooks** — `useStdb` (connection state via `useSyncExternalStore`), `useTable` (table subscriptions)
- **OIDC context** — `react-oidc-context` + `oidc-client-ts` for auth (see `lib/auth.ts`)
- **Local component state** — `useState` for UI-only state (sidebar open/close, etc.)

### Auth

OIDC via `react-oidc-context`. Authority defaults to `https://auth.spacetimedb.com/oidc`. Override with:
- `VITE_OIDC_AUTHORITY`
- `VITE_OIDC_CLIENT_ID`

Supports GitHub and Google providers. The `/callback` route handles the OIDC redirect.

### Styling

Tailwind CSS v4 via `@tailwindcss/vite` plugin. No config file — uses CSS-first configuration in `app.css`. Path alias `@/` maps to `src/`.

### Dev Proxy

`vite.config.ts` proxies `/api/*` to `http://localhost:8787` (spike-edge). Start spike-edge with `npm run dev` in `packages/spike-edge` before running the frontend for full-stack local dev.

## Environment Variables

| Variable | Default | Purpose |
|----------|---------|---------|
| `VITE_STDB_URL` | `ws://localhost:3000/stdb/ws` | SpacetimeDB WebSocket URL |
| `VITE_OIDC_AUTHORITY` | `https://auth.spacetimedb.com/oidc` | OIDC authority |
| `VITE_OIDC_CLIENT_ID` | `spike-platform-client-id` | OIDC client ID |

## Code Quality Rules

- TypeScript strict mode
- Never use `any` — use `unknown` or proper types
- Never add `eslint-disable` or `@ts-ignore` comments
- `routeTree.gen.ts` is auto-generated — never edit manually
- Tests use `@testing-library/react` + `vitest` with `jsdom` environment
- All new components should have a corresponding `.test.tsx`
