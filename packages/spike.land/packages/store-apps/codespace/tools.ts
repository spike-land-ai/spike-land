...
const CodeSpaceIdZ = z.string().min(1).max(100).regex(/^[a-zA-Z0-9_.-]+$/);
...
export const codespaceTools: StandaloneToolDefinition[] = [
...

  {
    ...
    handler: async (input: never): Promise<CallToolResult> => {
      ...
      validateCodeSpaceId(codespace_id);
      try {
        const { getSession, upsertSession } = await import(
          "@/lib/codespace/session-service"
        );
        const { transpileCode } = await import("@/lib/codespace/transpile");
...

  {
    ...
    handler: async (input: never, ctx: ServerContext): Promise<CallToolResult> => {
      ...
      const serviceToken = ctx.env.SPIKE_LAND_SERVICE_TOKEN
        || ctx.env.SPIKE_LAND_API_KEY || "";
      ...
      if (app_id) {
        ...
        return textResult(
          `**Linked!** App ${result.data?.name} → codespace ${codespace_id}`,
        );
      }
      if (!app_name) {
    ...
    handler: async (input: never): Promise<CallToolResult> =>
      safeToolCall("fs_write", async () => {
        ...
        if (newTotal > MAX_TOTAL_SIZE_BYTES) {
          return errorResult(
            `Total size limit (50MB) exceeded for codespace "${codespace_id}".`,
          );
        }
