...
export function registerReviewPrTools(registry: ToolRegistry, _userId: string): void {
  registry.register({
    ...
    description:
      "Get the diff for a pull request or branch. Returns file list with addition/deletion counts, total changes, and key files.",
    category: "review-pr",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: GetDiffSchema.shape,
    handler: async (
      { pr_number, branch, file_filter }: z.infer<typeof GetDiffSchema>,
  ...
  registry.register({
    ...
    description:
      "Suggest a concrete fix for a code issue at a specific file and line. Returns before/after code, explanation, and a confidence score.",
    category: "review-pr",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: SuggestFixSchema.shape,
    handler: async (
      { file_path, line_number, issue_description }: z.infer<
        typeof SuggestFixSchema
      >,
    ): Promise<CallToolResult> =>
  ...
  registry.register({
    ...
    description:
      "Check one or more source files against project conventions: no-any, no-ts-ignore, no-eslint-disable, no-console, prisma import pattern, naming conventions.",
    category: "review-pr",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: CheckConventionsSchema.shape,
    handler: async (
      { file_paths }: z.infer<typeof CheckConventionsSchema>,
    ): Promise<CallToolResult> =>
  ...
  registry.register({
    ...
    description:
      "Scan source files for security vulnerabilities. Returns findings with severity, CWE reference, location, description, and fix suggestion.",
    category: "review-pr",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: SecurityScanSchema.shape,
    handler: async (
      { file_paths, scan_type = "quick" }: z.infer<typeof SecurityScanSchema>,
    ): Promise<CallToolResult> =>
