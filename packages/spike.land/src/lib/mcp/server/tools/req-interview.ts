...
const BAZDMEG_QUESTIONS: ReadonlyArray<{ id: string; question: string; }> = [
    ...
    {
        id: "explainability",
        question: "Can you explain this approach to a teammate? If not, stop and learn more.",
    },
];
...
const AnswerItem = z.object({
    question_id: z.string().min(1).describe(
        "The question ID to answer (e.g. 'problem', 'data').",
    ),
...
export function registerReqInterviewTools(
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("interview_start", "Start a BAZDMEG requirements interview session. Creates an interview with 7 structured questions that must be answered before a specification can be generated.", {
                project_name: z.string().min(1).describe(
                    "Name of the project being specified.",
                ),
                initial_description: z.string().min(1).describe(
                    "Brief description of what the project should accomplish.",
                ),
            })
            ...
            .handler(async ({ input, ctx: _ctx }) => {
                ...
                return safeToolCall("interview_start", async () => {
                    const id = randomUUID();
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("interview_submit", "Submit answers to BAZDMEG interview questions. You may answer one or several questions at a time.", {
            ...
            .handler(async ({ input, ctx: _ctx }) => {
                ...
                return safeToolCall("interview_submit", async () => {
                    const interview = interviews.get(interview_id);
                    if (!interview) {
                        return {
                            content: [{
                                type: "text",
                                text: `Interview "${interview_id}" not found.`,
                            }],
                            isError: true,
                        };
                    }
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("interview_generate_spec", "Generate a structured specification from a completed BAZDMEG interview. All 7 questions must be answered first.", {
                interview_id: z.string().min(1).describe(
                    "ID of the completed interview session.",
                ),
            })
            .meta({ category: "orchestration", tier: "free" })
            .handler(async ({ input, ctx: _ctx }) => {
                ...
                return safeToolCall("interview_generate_spec", async () => {
                    const interview = interviews.get(interview_id);
                    if (!interview) {
                        return {
                            content: [{
                            ...
                            content: [
                                {
                                    type: "text",
                                    text:
                                        `Interview is incomplete. Unanswered questions: ${missing}. Answer all 7 questions before generating a spec.`,
                                },
                            ],
                            isError: true,
                        };
