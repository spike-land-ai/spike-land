...
export function registerMcpAnalyticsTools(
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("mcp_tool_usage_stats", "Get analytics on MCP tool usage: most-called tools, total invocations, average response time, and error rate. Filter by time period (24h, 7d, 30d, all) and optional category.", {
            ...
            .handler(async ({ input, ctx }) => {
                ...
                return safeToolCall("mcp_tool_usage_stats", async () => {
                    ...
                    if (totalCalls === 0) {
                        return textResult(
                            `No tool invocations found for the ${periodLabel(period ?? "7d")}${category ? ` in category "${category}"` : ""
                            }.`,
                        );
                    }
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("mcp_generate_docs", "Generate markdown documentation for a specific MCP tool or all tools in a category. Includes description, input parameters, example usage, and error codes.", {
