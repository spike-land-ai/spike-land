...
export function validateTemplate(template: string): string | null {
    ...
    if (allVars.length !== validVars.length) {
        ...
        return `Invalid template variables: ${invalid.join(", ")
            }. Only {{secrets.KEY}} and {{input.FIELD}} are allowed.`;
    }
    return null;
}
...
export function registerToolFactoryTools(
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("register_tool", "Register a custom tool with a declarative HTTP proxy handler. "
                ...
                try {
                            ...
                            content: [
                                {
                                    type: "text",
                                    text:
                                        `Tool limit reached (${count}/${limit}). Upgrade to Premium for up to ${PREMIUM_TOOL_LIMIT} tools.`,
                                },
                            ],
                            isError: true,
                        };
                    ...
                    if (urlError) {
                        return {
                            content: [{
                                type: "text",
                                text: `Invalid handler URL: ${urlError}`,
                            }],
                            isError: true,
                        };
                    }
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("test_tool", "Execute a registered tool with test inputs. Only works with DRAFT or PUBLISHED tools.", {
            ...
            .handler(async ({ input, ctx: _ctx }) => {
                ...
                try {
                    ...
                    const spec = tool.handlerSpec as unknown as z.infer<
                        typeof HandlerSpecSchema
                    >;
                                    ...
                                    content: [
                                        {
                                            type: "text",
                                            text:
                                                `Missing or unapproved secret: ${ref}. Store and approve it with vault tools first.`,
                                        },
                                    ],
                                    isError: true,
                                };
                            ...
                            content: [
                                {
                                    type: "text",
                                    text:
                                        `Resolved URL is invalid: ${resolvedUrlError}. Template variables may have injected a disallowed URL.`,
                                },
                            ],
                            isError: true,
                        };
                    ...
                    try {
                        ...
                        const truncated = responseText.length > 1_000_000
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("publish_tool", "Publish a draft tool to make it available for use.", {
                tool_id: z.string().min(1),
            })
            .meta({ category: "tools", tier: "free" })
            .handler(async ({ input, ctx: _ctx }) => {
                    ...
                    if (!tool) {
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: "Tool not found or you don't have access.",
                                },
                            ],
                            isError: true,
                        };
                            ...
                            content: [
                                {
                                    type: "text",
                                    text:
                                        `Tool "${tool.name}" is ${tool.status}, not DRAFT. Only draft tools can be published.`,
                                },
                            ],
                            isError: true,
                        };
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("disable_tool", "Disable a registered tool.", {
                tool_id: z.string().min(1),
            })
            .meta({ category: "tools", tier: "free" })
            .handler(async ({ input, ctx: _ctx }) => {
