...
export function registerCodespaceTemplateTools(
    ...
    registry.registerBuilt(
        freeTool(_userId)
            .tool("codespace_list_templates", "Browse available project templates for codespace creation.\n"
            ...
            .meta({ category: "codespace-templates", tier: "free" })
            .handler(async ({ input, ctx: _ctx }) => {
                ...
                const lines = filtered.map(t =>
                    ...
                    + `  ${t.description}\n`
                    + `  Files: ${t.fileCount} | `
                    + `Dependencies: ${t.dependencies.length === 0 ? "none" : t.dependencies.map(d => d.name).join(", ")
                    }`
                );
    ...
    registry.registerBuilt(
        freeTool(_userId)
            .tool("codespace_create_from_template", "Create a new codespace pre-populated from a project template.\n"
            ...
            .meta({ category: "codespace-templates", tier: "free" })
            .handler(async ({ input, ctx: _ctx }) => {
                const { template_id, name, description } = input;
    ...
    registry.registerBuilt(
        freeTool(_userId)
            .tool("codespace_get_dependencies", "List npm dependencies inferred from a codespace's source code.\n"
    ...
    registry.registerBuilt(
        freeTool(_userId)
            .tool("codespace_add_dependency", "Record an npm package dependency for a codespace.\n"
            ...
            .meta({ category: "codespace-templates", tier: "free" })
            .handler(async ({ input, ctx: _ctx }) => {
                const { codespace_id, package_name, version, dev } = input;
