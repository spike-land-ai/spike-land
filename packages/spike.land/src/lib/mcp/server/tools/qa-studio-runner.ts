/**
 * QA Studio Runner MCP Tools
 *
 * Browser automation tools that directly drive Playwright APIs.
 * Registered as part of the QA Studio tool set.
 *
 * Tools:
...
export function registerQaStudioRunnerTools(
  ...
  registry.register({
    name: "qa_console",
    description:
      "Capture console messages from the current browser tab. Filter by log level.",
    category: CATEGORY,
    tier: TIER,
    alwaysEnabled: true,
    inputSchema: {
  ...
  registry.register({
    name: "qa_tabs",
    description:
      "Manage browser tabs: list open tabs, switch between them, or close a tab.",
    category: CATEGORY,
    tier: TIER,
    alwaysEnabled: true,
    inputSchema: {
        ...
        const audit = await tab.page.evaluate(() => {
          ...
          if (smallTargets.length > 0) {
            issues.push({
              ...
              details:
                `Found ${smallTargets.length} interactive elements smaller than 44x44px.`,
              impact: "moderate",
            });
          }
