...
export function registerSwarmMonitoringTools(
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("swarm_get_metrics", "Get swarm performance metrics: agents spawned, tasks completed, average task duration, success rate, and resource usage for a given time period.", {
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("swarm_get_cost", "Get token usage and cost breakdown for the swarm. Filter by agent_id for per-agent breakdown, or omit for swarm-wide totals.", {
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("swarm_replay", "Replay an agent's execution history step-by-step. Returns timestamped tool calls and results with optional step range filtering.", {
            ...
            .handler(async ({ input, ctx }) => {
                ...
                if (slicedLogs.length === 0) {
                    return textResult(
                        `No steps in range [${startIdx}, ${to_step ?? allLogs.length - 1
                        }] for agent ${agent_id}.`,
                    );
                }

                let text = `**Replay: ${agent.displayName}** (steps ${startIdx}–${startIdx + slicedLogs.length - 1
