...
export function registerNetsimTools(
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("netsim_create_topology", "Create a network topology with N nodes connected in a full mesh. All links start as 'up' with 0 latency and 0 loss. Returns topology ID and node list.", {
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("netsim_set_link_state", "Change the state of a network link between two nodes. States: up (normal), partitioned (all messages dropped), slow (adds latency), lossy (random drops).", {
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("netsim_partition_node", "Fully partition a node from the network. All links to and from the node are set to 'partitioned'. Messages will be dropped.", {
                topology_id: z.string().min(1).describe("ID of the network topology."),
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("netsim_heal_node", "Heal a partitioned node, restoring all its links to 'up' state with 0 latency and 0 loss.", {
                topology_id: z.string().min(1).describe("ID of the network topology."),
                node_id: z.string().min(1).describe("Node ID to partition or heal."),
            })
            .meta({ category: "netsim", tier: "free" })
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("netsim_send_message", "Send a message from one node to another through the simulated network. The message will be subject to the link's current state (delivered, delayed, or dropped).", {
                topology_id: z.string().min(1).describe("ID of the network topology."),
    ...
    registry.registerBuilt(
        freeTool(userId)
            .tool("netsim_tick", "Advance the simulation clock by N rounds, delivering pending messages based on link states. Returns delivery statistics.", {
                topology_id: z.string().min(1).describe("ID of the network topology."),
                rounds: z
                    .number()
                    .int()
