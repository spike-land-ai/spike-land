...
export async function generateMetadata(
  ...
  try {
    ...
    // DB connection timeout — return fallback metadata
    return { title: `${path.join(" ")} | LearnIt` };
  }
}
...
export default async function LearnItTopicPage({ params }: PageProps) {
  ...
  let content;
  try {
    content = await getLearnItContentCached(slug);
  } catch {
    // DB connection timeout — treat as "not found" to show streaming UI
    content = null;
  }
...
function Breadcrumbs(
  { items }: { items: { label: string; href: string; isLast: boolean; }[]; },
) {
  return (
    <Breadcrumb>
      <BreadcrumbList>
