...
if (fromIndex === -1 || !args[fromIndex + 1]) {
  console.error(
    "Usage: npx tsx prisma/scripts/restore-database.ts --from <backup-directory>",
  );
  process.exit(1);
}
...
async function restore() {
  ...
  const restoreOrder = [
    ...
    },
    {
      ...
      restore: async () => {
        const data = loadBackupFile<Prisma.EnhancedImageUncheckedCreateInput>(
          "enhanced_images",
        );
        let count = 0;
        for (const record of data) {
    ...
    },
    {
      ...
      restore: async () => {
        const data = loadBackupFile<
          Prisma.ImageEnhancementJobUncheckedCreateInput
        >(
    ...
    },
    {
      name: "api_keys",
      restore: async () => {
        const data = loadBackupFile<Prisma.ApiKeyUncheckedCreateInput>(
          "api_keys",
        );
        let count = 0;
        for (const record of data) {
          try {
            ...
            console.log(
              `   ⚠️  ApiKey: ${e instanceof Error ? e.message.substring(0, 50) : e}`,
            );
          }
        }
        return count;
    ...
    },
    {
      ...
      restore: async () => {
        const data = loadBackupFile<
          Prisma.FeaturedGalleryItemUncheckedCreateInput
        >(
