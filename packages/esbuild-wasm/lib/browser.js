...
`,
    ...
    if (supported) {
      for (let key in supported) {
        ...
        if (typeof value !== "boolean") {
          throw new Error(
            `Expected value for supported ${quote(
              key,
  ...
  function flagsForBuildOptions(callName, options, isTTY, logLevelDefault, writeDefault) {
    ...
    if (resolveExtensions) {
      flags.push(
        `--resolve-extensions=${validateAndJoinStringArray(
          resolveExtensions,
          "resolve extension",
        )}`,
      );
    ...
    let handleIncomingPacket = (bytes) => {
      if (isFirstPacket) {
        ...
        if (binaryVersion !== "0.27.3") {
          throw new Error(
            `Cannot start service: Host version "${"0.27.3"}" does not match binary version ${quote(
              binaryVersion,
            )}`,
          );
        }
