...
vi.mock("better-auth/plugins", () => ({
    ...
    createAuthEndpoint: vi.fn(
        (_path: string, _opts: unknown, handler: unknown) => handler
    ),
}));
...
describe("createAuth", () => {
    ...
    it("includes magicLink and qr-auth plugins", async () => {
        ...
        const qrPlugin = callArg.plugins.find(
            (p: { id?: string }) => p.id === "qr-auth"
        );
    ...
    it("uses drizzle-orm/d1 with the AUTH_DB from env", async () => {
        ...
        createAuth(env);
        expect(drizzle).toHaveBeenCalledWith(mockDb, expect.objectContaining({ schema: expect.any(Object) }));
    });
