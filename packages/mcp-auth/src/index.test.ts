...
const mockHandleRequest = vi.fn(async (_req: Request) =>
...
vi.mock("@modelcontextprotocol/sdk/server/mcp.js", () => ({
    McpServer: vi.fn().mockImplementation(() => ({
        tool: vi.fn((name: string, description: string, schema: unknown, handler: (args: Record<string, unknown>) => Promise<unknown>) => {
            registeredTools.push({ name, description, schema: schema as Record<string, unknown>, handler });
        }),
...
describe("MCP tool: verify-session", () => {
    ...
    it("returns valid:false when session is null", async () => {
        ...
        const result = await tool!.handler({ sessionToken: "invalid-token" }) as { content: Array<{ text: string }> };
    ...
    it("returns valid:false when sessionResult has no session property", async () => {
        ...
        const tool = await getVerifySessionTool();
        const result = await tool!.handler({ sessionToken: "token" }) as { content: Array<{ text: string }> };
        const data = JSON.parse(result.content[0].text);
        expect(data.valid).toBe(false);
    });
    ...
    it("returns valid:true with user and session when session exists", async () => {
        ...
        const tool = await getVerifySessionTool();
        const result = await tool!.handler({ sessionToken: "valid-token" }) as { content: Array<{ text: string }> };
        const data = JSON.parse(result.content[0].text);
    ...
    it("returns text content type in response", async () => {
        mockGetSession.mockResolvedValueOnce(null);
        const tool = await getVerifySessionTool();
        const result = await tool!.handler({ sessionToken: "tok" }) as { content: Array<{ type: string }> };
...
describe("MCP tool: get-user-by-email", () => {
    ...
    it("returns found:false when user does not exist", async () => {
        ...
        expect(tool).toBeDefined();
        const result = await tool!.handler({ email: "notfound@example.com" }) as { content: Array<{ text: string }> };
        const data = JSON.parse(result.content[0].text);
    ...
    it("returns found:true with user fields when user exists", async () => {
        ...
        const tool = await getEmailTool();
        const result = await tool!.handler({ email: "alice@example.com" }) as { content: Array<{ text: string }> };
        const data = JSON.parse(result.content[0].text);
    ...
    it("only returns id, email, name, role — not image", async () => {
        ...
        const tool = await getEmailTool();
        const result = await tool!.handler({ email: "bob@example.com" }) as { content: Array<{ text: string }> };
        const data = JSON.parse(result.content[0].text);
    ...
    it("returns null role when user has no role set", async () => {
        ...
        const tool = await getEmailTool();
        const result = await tool!.handler({ email: "charlie@example.com" }) as { content: Array<{ text: string }> };
        const data = JSON.parse(result.content[0].text);
        expect(data.found).toBe(true);
    ...
    it("returns text content type", async () => {
        mockFindFirst.mockResolvedValueOnce(null);
        const tool = await getEmailTool();
        const result = await tool!.handler({ email: "x@example.com" }) as { content: Array<{ type: string }> };
        expect(result.content[0].type).toBe("text");
    });
});
