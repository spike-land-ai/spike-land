...
- Browse 80+ tools in the store, organized by category and tier
- Connect via `claude mcp add spike-land --transport http https://spike.land/mcp` in under 60 seconds
...
The fastest path to a working MCP setup is shorter than you expect.

You need two things:

1. **Node.js 18+** — check with `node -v`
2. **A spike.land account** — [sign up here](https://spike.land/login), free, no credit card
...
Go to [spike.land/login](https://spike.land/login).
...
Tool calls are free if the tool does not call a model. The ones that do — image analysis, code explanation, content generation — need either your own API keys or proxy credits. Here is how to handle that.
...
**Migrating from Next.js?** — The [migration service](/blog/nextjs-migration-service) explains the method and what the tiers cover.
...
The first `npx @spike-land-ai/spike-cli shell` download takes 10-20 seconds. Subsequent runs use the cache. Install globally to skip this: `npm install -g @spike-land-ai/spike-cli`.
...
The setup is done. The interesting part starts now.
