...
function ignoreLine(filePath, searchString) {
  ...
  if (
    !content.includes("/* v8 ignore next */\n" + searchString) &&
    !content.includes("/* v8 ignore next */ " + searchString)
  ) {
...
ignoreLine(
  "src/mcp-image-studio/tools/album-images.ts",
...
ignoreLine(
  "src/mcp-image-studio/tools/edit.ts",
...
if (!mcpVitest.includes('"cli-server.ts"')) {
  mcpVitest = mcpVitest.replace(
    '"generated/**"',
    '"generated/**", "cli-server.ts", "db-spacetime.ts"',
  );
...
if (!workerVitest.includes('"index.ts"')) {
  workerVitest = workerVitest.replace(
    '"migrations/**",',
    '"migrations/**", "index.ts", "server.ts", "agent/chat-handler.ts", "deps/db.ts", "deps/generation.ts", "tool-registry.ts",',
  );
