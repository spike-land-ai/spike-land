...
replaceBlock(
  ...
  (block) => {
    return (
      `export const getApiStoreToolsHandler = async (c: import("hono").Context<{ Bindings: Env; Variables: Variables }>) => {` +
...
replaceBlock(
  ...
  (block) => {
    return (
      `export const oauthAuthorizationServerHandler = (c: import("hono").Context<{ Bindings: Env; Variables: Variables }>) => {` +
      block.substring(
        'app.get("/.well-known/oauth-authorization-server", (c) => {'.length,
        block.length - 3,
      ) +
...
replaceBlock(
  ...
  (block) => {
    return (
      `export const oauthProtectedResourceMcpHandler = (c: import("hono").Context<{ Bindings: Env; Variables: Variables }>) => {` +
      block.substring(
        'app.get("/.well-known/oauth-protected-resource/mcp", (c) => {'.length,
        block.length - 3,
      ) +
...
replaceBlock(
  ...
  (block) => {
    return (
      `export const oauthDeviceApproveHandler = async (c: import("hono").Context<{ Bindings: Env; Variables: Variables }>) => {` +
      block.substring(
        'app.post("/oauth/device/approve", authMiddleware, async (c) => {'.length,
        block.length - 3,
      ) +
...
replaceBlock('app.get("/mcp", async (c, next) => {', "  return next();\n});", (block) => {
  return (
    `export const mcpGetHandler = async (c: import("hono").Context<{ Bindings: Env; Variables: Variables }>, next: import("hono").Next) => {` +
    block.substring('app.get("/mcp", async (c, next) => {'.length, block.length - 3) +
    `};\napp.get("/mcp", mcpGetHandler);`
  );
});

replaceBlock('app.all("/api/auth/*", async (c) => {', "  });\n});", (block) => {
  return (
    `export const apiAuthAllHandler = async (c: import("hono").Context<{ Bindings: Env; Variables: Variables }>) => {` +
...
replaceBlock(
  ...
  (block) => {
    return (
      `export const apiCatchAllHandler = (c: import("hono").Context<{ Bindings: Env; Variables: Variables }>) => {` +
