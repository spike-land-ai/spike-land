...
import { LRUCache } from "../../../../../src/frontend/monaco-editor/core-logic/lib/lru-cache/index";
...
describe("LRUCache", () => {
  ...
  describe("constructor validation", () => {
    it("should throw error when max is not a positive integer", () => {
      expect(() => new LRUCache({ max: -1 })).toThrow("max option must be a nonnegative integer");
      expect(() => new LRUCache({ max: 1.5 })).toThrow("max option must be a nonnegative integer");
    ...
    it("should throw error when maxSize is invalid", () => {
      expect(() => new LRUCache({ maxSize: -100 })).toThrow(
    ...
    it("should throw error when sizeCalculation is set without maxSize", () => {
      ...
      ).toThrow("cannot set sizeCalculation without setting maxSize or maxEntrySize");
    });
  ...
  describe("delete operation", () => {
    ...
    it("should call dispose callback on delete", () => {
      ...
      cache.set("key1", { value: "value1" });
      cache.delete("key1");
      expect(dispose).toHaveBeenCalledWith({ value: "value1" }, "key1", "delete");
    });
  });
  ...
  describe("fetch method", () => {
    it("should fetch value when not in cache", async () => {
      ...
      expect(fetchMethod).toHaveBeenCalledWith("key1", undefined, expect.any(Object));
    });
  ...
  describe("memo method", () => {
    ...
    it("should throw when no memoMethod provided", () => {
      ...
      expect(() => cache.memo("key1")).toThrow("no memoMethod provided to constructor");
    });
  });
