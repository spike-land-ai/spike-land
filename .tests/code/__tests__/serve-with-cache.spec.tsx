...
describe("serveWithCache", () => {
  ...
  it("should handle asset fetcher errors", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith("Asset fetch error:", expect.any(Error));
  ...
  it("should handle cache put failures", async () => {
    ...
    await vi.waitFor(() => {
      expect(consoleErrorSpy).toHaveBeenCalledWith("Cache put error:", expect.any(Error));
    });
