  ...
  return {
    GoogleGenAI: class {
      models = {
        generateContent: vi
          .fn()
          .mockImplementation((opts: { contents: Array<{ parts: Array<{ text?: string }> }> }) => {
...
describe("generation", () => {
  ...
  it("createGenerationJob", async () => {
    const gen = createGeminiGeneration(
      { GEMINI_API_KEY: "k" } as unknown as Env,
      db,
      credits,
      storage,
    );
  ...
  it("createGenerationJob error", async () => {
    ...
    await expect(
      gen.createGenerationJob({ userId: "u", prompt: "p", tier: "FREE" } as unknown as Parameters<
        ImageStudioDeps["generation"]["createGenerationJob"]
      >[0]),
    ).rejects.toThrow();
  });
  ...
  it("createAdvancedGenerationJob", async () => {
    ...
    const res = await gen.createAdvancedGenerationJob({
      ...
      NonNullable<ImageStudioDeps["generation"]["createAdvancedGenerationJob"]>
    >[0]);
    expect(res.success).toBe(true);
  });
