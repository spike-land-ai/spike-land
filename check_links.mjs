    ...
    while ((match = linkRegex.exec(content)) !== null) {
        ...
        if (link.startsWith('http')) {
            ...
            promises.push(new Promise(resolve => {
                ...
                const req = client.get(link, { timeout: 5000 }, (res) => {
                    if (res.statusCode >= 400 && res.statusCode !== 403) { // Ignore 403 as it might be bot blocking
...
Promise.all(promises).then(results => {
    ...
    if (broken.length === 0) {
        ...
        broken.forEach(b => console.log(`${b.file} | Link: ${b.link} | Status: ${b.status} | Type: ${b.type}`));
    }
});
